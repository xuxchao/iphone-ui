...
const vueI18nHandler = function () {
  ...
  if (typeof vuei18n === 'function' && !!Vue.locale) {
    if (!merged) {
      ...
      Vue.locale(
        Vue.config.lang,
        Object.assign(Vue.config.lang === 'zn-CH' ? znCH : {}, Vue.locale(Vue.config.lang) || {}, defaultMessages)
      )
    }
  ...
  if (pars && conf) {
    ...
    pars.forEach((c, i) => {
      ...
      templ = templ.replace(reg, (a, b) => {
        return getArgType(conf[c]) === 'function' ? conf[c]() : (conf[c] !== undefined ? conf[c] : (b || a))
      })
    })
  }
  return templ
